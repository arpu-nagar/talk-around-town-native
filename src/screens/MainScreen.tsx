...
const MapViewModal = React.memo(function MapViewModal({
  ...
  const addLocation = useCallback(async () => {
    if (!newLocation || !name.trim() || !description.trim() || !selectedOption) {
      Alert.alert('Missing Information', 'Please enter a name, description and select a location type.');
      return;
    }
    try {
      const res = await fetchWithAuth(`${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.ADD_LOCATION}`, {
        method: 'POST',
        headers: {
  ...
  }, [newLocation, name, description, selectedOption, token, onRefresh, reset, onClose]);
    ...
    <Modal visible={visible} animationType="slide" presentationStyle="fullScreen" onRequestClose={onClose}>
      <SafeAreaView style={styles.mapModalContainer}>
        ...
        <View style={styles.mapHeader}>
          ...
          <Text style={styles.mapHeaderTitle}>Add Location</Text>
          <View style={{ width: 24 }} />
        </View>
        ...
        {initialRegion && (
          ...
          >
            {newLocation && <Marker coordinate={newLocation} title="New Location" pinColor="#4A90E2" />}
            {locations.map((loc, i) => (
              <React.Fragment key={`loc-${i}`}>
                <Marker
                  ...
                  pinColor="#FF4B4B"
                />
                <Circle center={loc} radius={100} strokeColor="rgba(255,75,75,0.5)" fillColor="rgba(255,75,75,0.1)" />
              </React.Fragment>
            ))}
          </MapView>
        )}
...
function buildAgeFromInputs(yy: string, mm: string) {
  ...
  const getQuickLocation = useCallback(async (): Promise<Location> => {
    ...
    return new Promise((resolve) => {
      ...
      Geolocation.getCurrentPosition(
        ...
        { enableHighAccuracy: false, timeout: STARTUP_CONFIG.QUICK_LOCATION_TIMEOUT - 500, maximumAge: 60000 }
      );
    });
  ...
  const refreshDataInBackground = useCallback(async () => {
    ...
    try {
      // Locations
      const locationsResponse = await fetchWithAuth(`${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.LOCATIONS}`, {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
      ...
      if (locationsResponse.ok) {
        ...
        if (Array.isArray(data.locations) && Array.isArray(data.details)) {
          ...
          await saveToCache(LOCATION_CONFIG.CACHE_KEYS.USER_LOCATIONS, { locations: data.locations, details: data.details });
        }
      }

      // Children
      const childrenResponse = await fetchWithAuth(`${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.CHILDREN}`, {
      ...
      // Preference profile
      const profileResponse = await fetchWithAuth(`${API_ENDPOINTS.BASE_URL}/api/personalization/profile`, {
        headers: { Authorization: `Bearer ${userInfo.access_token}` },
      });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Cached first
        const cachedLocs = await loadFromCache(LOCATION_CONFIG.CACHE_KEYS.USER_LOCATIONS);
        if (cachedLocs) {
          setLocations(cachedLocs.locations || []);
          setDetails(cachedLocs.details || []);
        }
        const cachedKids = await loadFromCache(LOCATION_CONFIG.CACHE_KEYS.CHILDREN_INFO);
        if (cachedKids) setUserChildren(cachedKids);
        const liked = await loadFromCache('likedTips'); if (Array.isArray(liked)) setLikedTips(liked);
        const disliked = await loadFromCache('dislikedTips'); if (Array.isArray(disliked)) setDislikedTips(disliked);
        const savedPrefs = await AsyncStorage.getItem('contentPreferences');
        if (savedPrefs) {
          const parsed = JSON.parse(savedPrefs);
          if (Array.isArray(parsed) && parsed.length) setContentPreferences(parsed);
        }
  ...
  useEffect(() => {
    const initVoice = async () => {
      try {
        ...
        Voice.onSpeechEnd = () => { if (isListening) Voice.start('en-US'); };
      } catch (e) {
  ...
  const flushAIReactionsQueue = useCallback(async () => {
    ...
    try {
      const res = await fetchWithAuth(`${API_ENDPOINTS.BASE_URL}/api/personalization/ai-interactions/batch`, {
        method: 'POST',
  ...
  useEffect(() => {
    const unsub = NetInfo.addEventListener((s) => { if (s.isConnected) flushAIReactionsQueue(); });
  ...
  const toggleListening = async () => {
    try {
      if (isListening) {
        ...
        if (!ok) return Alert.alert('Error', 'Voice recognition is not available on this device.');
        setSearchText('');
        lastResult.current = '';
  ...
  const tinyHash = (s: string) => [...s].reduce((h, c) => (((h << 5) - h) + c.charCodeAt(0)) | 0, 0).toString();
  const tipKey = (t: Tip) => (typeof t.id === 'number' && !t.isGenerated ? `db:${t.id}` : `ai:${tinyHash(`${t.title||''}|${t.body||''}|${t.details||''}`)}`);
...
const HELP_WORDS = /\b(tips?|how to|how-to|ideas?|tricks?|help|advice|guide|activities?)\b/i;
...
const DIY_TERMS = [
  'diy','craft','crafts','arts and crafts','art project','project',
  'science experiment','experiments','stem activity','stem activities',
  'maker','lego','sensory','sensory play','sensory bin',
  'playdough','play-dough','slime','origami','paper craft','rainy day'
];

const NEUTRAL_PARENTING_ENV = [
  'grocery','supermarket','shopping','store','errands','restaurant','waiting room','library','park',
  'bath','bath time','shower','hygiene','tooth brushing','toothbrush',
  'car seat','car ride','road trip','flight','plane','airport','bus','train','travel',
...
const CHILD_TERMS = [
  'child','kid','toddler','baby','infant','newborn','teen','teenager','preteen',
  'son','daughter','my boy','my girl','my kid','my child','my toddler','my baby',
  'students','kids','children','parent','parenting','daycare','preschool','school'
];


// Common parenting topics you want to allow through
const PARENTING_TOPICS = [
  'bedtime','sleep','tantrum','behavior','discipline','potty','toilet','diaper',
  'screen time','homework','reading','milestone','play','activity','activities',
  'language','speech','feeding','picky eater','vegetables','routine','chores',
  'bullying','friends','social','sharing','attention','focus','study','grades','sleep','tantrum','meltdown','behavior','discipline','routine',
  'screen time','homework','reading','literacy','milestone','play','activity','activities',
  'language','speech','feeding','picky eater','vegetables','toilet','potty','diaper',
  'social','sharing','bullying','focus','study','grades','friends',
  // hygiene & self-care
  'bath','bath time','shower','hygiene','tooth brushing','toothbrush','toileting',
  // out-and-about / errands
  'grocery','supermarket','shopping','store','errands','restaurant','waiting room','library','park',
  // travel & logistics
  'car seat','car ride','road trip','flight','plane','airport','bus','train','travel',
  // transitions / routines
  'morning routine','evening routine','after school','bedtime routine','nap','naptime',
...
const similar = (a: string, b: string, maxDist = 1) => levenshtein(a, b) <= maxDist;
...
function resolveChildrenFromQuery(query: string, children: Child[]) {
  ...
  for (const c of children) {
    ...
    const exact = words.has(name) || q.includes(`${name}'s`) || q.includes(`my ${name}`);
    if (exact) { hits.push({ child: c, how: 'exact' }); continue; }
  ...
  for (const h of hits) {
    ...
    if (!uniq[key] || (uniq[key].how === 'fuzzy' && h.how === 'exact')) uniq[key] = h;
  }
...
function resolveChildrenAndUnknownNames(query: string, children: Child[]) {
  ...
  for (const t of tokens) {
    ...
    for (const c of children) {
      ...
      if (!name) continue;
      if (t === name) { matched = c; break; }
      // tiny fuzzy room
      const dist = levenshtein(t, name);
      if (dist <= (name.length <= 5 ? 1 : 2)) { matched = c; break; }
    }
    if (matched) {
      if (!known.find(k => (k.nickname || '').toLowerCase() === (matched!.nickname || '').toLowerCase())) {
        known.push(matched);
      }
    } else {
...
const getPersonalizedTips = async () => {
  ...
  if (!query) return Alert.alert('Input Required', 'Please enter what you need help with');
  ...
  try {
    ...
    if (!res.ok) {
      if (data.error === 'safety' || data.error === 'non_parenting') {
        Alert.alert('We only provide parenting tips', data.message || 'Please ask a parenting-related question.');
        return;
      }
      throw new Error(`Server responded with ${res.status}: ${data.message || 'Unknown error'}`);
    }
    ...
    Alert.alert('Error', 'Failed to get advice. Please check your connection and try again.');
  } finally {
  ...
  const showParentingExamples = (suggestions: string[]) => {
    ...
    Alert.alert(
      ...
      examples.slice(0, 6).map(ex => `• ${ex}`).join('\n'),
      [
  ...
  const speakTip = useCallback(async (tip: Tip) => {
    ...
    try {
      ...
      if (!audioUrl) {
        if (tip.audioUrl) {
          ...
          const res = await fetch(`${API_ENDPOINTS.ASSISTANT_BASE_URL}/generate-tip-audio`, {
            method: 'POST',
      ...
      currentSound.current = new Sound(audioUrl, '', (err) => {
        ...
        currentSound.current?.play((success) => {
          if (!success) Alert.alert('Error', 'Audio playback failed. Please try again.');
          setIsPlaying(false);
          setActiveAudioKey(null);
          currentSound.current?.release();
          currentSound.current = null;
  ...
  // Like/Dislike
  const isTipLiked = (tip: Tip) => likedTips.some((x) => tipKey(x) === tipKey(tip));
  const isTipDisliked = (tip: Tip) => dislikedTips.some((x) => tipKey(x) === tipKey(tip));
  const setLikedCache = async (arr: Tip[]) => saveToCache('likedTips', arr);
  const setDislikedCache = async (arr: Tip[]) => saveToCache('dislikedTips', arr);

  const postInteraction = async (tipId: number, interactionType: 'like' | 'dislike') => {
    const res = await fetchWithAuth(`${API_ENDPOINTS.BASE_URL}/api/personalization/interactions`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${userInfo.access_token}` },
  ...
  const queueAIInteraction = async (tip: Tip, reaction: 'like' | 'dislike') => {
    ...
    const entry = { key, reaction, title: tip.title, body: tip.body, details: tip.details, at: Date.now() };
  ...
  const setReaction = async (tip: Tip, reaction: 'like' | 'dislike') => {
    ...
    try {
      if (reaction === 'like') {
        ...
        setLikedTips(nextLikes); setDislikedTips(nextDislikes);
        await Promise.all([setLikedCache(nextLikes), setDislikedCache(nextDislikes)]);
      } else {
        const nextDislikes = isTipDisliked(tip) ? dislikedTips : [tip, ...dislikedTips];
        const nextLikes = likedTips.filter((t) => !sameKey(t, tip));
        setDislikedTips(nextDislikes); setLikedTips(nextLikes);
        await Promise.all([setDislikedCache(nextDislikes), setLikedCache(nextLikes)]);
      }

      if (typeof tip.id === 'number' && !tip.isGenerated) await postInteraction(tip.id, reaction);
        ...
        <View style={styles.tipCardShadow}>
          <LinearGradient colors={['#ffffff', '#f8f9fa']} style={styles.tipGradient}>
            <View style={styles.tipHeader}>
              <MaterialIcons name="auto-awesome" size={24} color="#6C63FF" style={{ marginRight: 12 }} />
...
>
  <MaterialIcons name={playing ? 'stop' : 'play-arrow'} size={16} color="#fff" />
  <Text style={styles.playButtonText}>{playing ? 'Stop' : 'Play'}</Text>
</TouchableOpacity>

              <TouchableOpacity style={{ marginLeft: 8, padding: 6, opacity: tip.isGenerated ? 0.4 : 1 }} onPress={() => setReaction(tip, 'like')}>
                <MaterialIcons name={isTipLiked(tip) ? 'favorite' : 'favorite-border'} size={22} color={isTipLiked(tip) ? '#FF3B30' : '#999'} />
              </TouchableOpacity>

              <TouchableOpacity style={{ marginLeft: 4, padding: 6, opacity: tip.isGenerated ? 0.4 : 1 }} onPress={() => setReaction(tip, 'dislike')}>
                <MaterialIcons name={isTipDisliked(tip) ? 'thumb-down' : 'thumb-down-off-alt'} size={22} color={isTipDisliked(tip) ? '#6C63FF' : '#999'} />
              </TouchableOpacity>
            </View>
          </LinearGradient>
        </View>
  ...
  const addLocation = async () => {
    if (!newLocation || !name.trim() || !description.trim() || !selectedOption) {
      return Alert.alert('Missing Information', 'Please enter a name, description and select a location type.');
    }
    try {
      const response = await fetchWithAuth(`${API_ENDPOINTS.BASE_URL}${API_ENDPOINTS.ADD_LOCATION}`, {
        method: 'POST',
        headers: { Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${userInfo.access_token}` },
        body: JSON.stringify({
          latitude: newLocation.latitude,
          longitude: newLocation.longitude,
          name,
...
/>
    ...
    <Modal visible={showTipsModal} animationType="slide" presentationStyle="pageSheet">
      <SafeAreaView style={styles.modalContainer}>
        <View style={styles.modalHeader}>
          ...
          <TouchableOpacity style={styles.closeModalButton} onPress={() => setShowTipsModal(false)}>
            <MaterialIcons name="close" size={24} color="#666" />
          </TouchableOpacity>
        </View>

        <ScrollView style={styles.modalContent} keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 28 }} showsVerticalScrollIndicator={false}>
  ...
  <View style={{flex:1, backgroundColor:'rgba(0,0,0,0.35)', justifyContent:'center', alignItems:'center', padding:20}}>
    <View style={{width:'100%', borderRadius:16, backgroundColor:'#fff', padding:16}}>
      <Text style={{fontSize:16, fontWeight:'600', marginBottom:8}}>
        Add age for {pendingUnknownName ? `"${pendingUnknownName}"` : 'child'}
      </Text>
      <Text style={{color:'#6b7280', marginBottom:12}}>
        We can personalize tips for this question using just an age (no need to save the child).
      </Text>

      <View style={{flexDirection:'row', gap:12}}>
        <View style={{flex:1}}>
          <Text style={{fontSize:13, color:'#6b7280'}}>Years</Text>
          <TextInput keyboardType="number-pad" value={ageYearsInput} onChangeText={setAgeYearsInput}
            style={{borderWidth:1, borderColor:'#e5e7eb', borderRadius:10, height:44, paddingHorizontal:12, marginTop:6}} />
        </View>
        <View style={{flex:1}}>
          <Text style={{fontSize:13, color:'#6b7280'}}>Months</Text>
          <TextInput keyboardType="number-pad" value={ageMonthsInput} onChangeText={setAgeMonthsInput}
            style={{borderWidth:1, borderColor:'#e5e7eb', borderRadius:10, height:44, paddingHorizontal:12, marginTop:6}} />
        </View>
      </View>

      <View style={{flexDirection:'row', justifyContent:'flex-end', marginTop:16}}>
        <TouchableOpacity onPress={() => { setAgePromptVisible(false); setPendingUnknownName(null); setAgeYearsInput(''); setAgeMonthsInput(''); }}
        ...
        <TouchableOpacity
          onPress={() => {
            const { pretty, yearsFloat } = buildAgeFromInputs(ageYearsInput, ageMonthsInput);
            if (yearsFloat <= 0 && ageMonthsInput.trim() === '' && ageYearsInput.trim() === '') {
              Alert.alert('Age required', 'Enter years and/or months.'); return;
            }
            if (pendingUnknownName) {
              setTempChildContext(prev => [{ name: pendingUnknownName, agePretty: pretty, ageYears: Math.max(0, parseInt(ageYearsInput||'0',10)||0) }, ...prev]);
            }
            setAgePromptVisible(false);
            setPendingUnknownName(null);
            setAgeYearsInput(''); setAgeMonthsInput('');
            // Optionally trigger the request again if you paused it
          }}
          style={{backgroundColor:'#4A90E2', borderRadius:10, paddingVertical:10, paddingHorizontal:14}}>
  ...
  // Header height so the blue only covers the ENACT area
      ...
      >
        ...
        <View style={styles.headerShadow}>
          <LinearGradient
           ...
           style={[styles.headerBar, { height: insets.top + 250 }]} 
          >
            <View style={styles.headerContent}>
              <View style={styles.brandingContainer}>
                <Text style={styles.appName}>ENACT</Text>
                <Text style={styles.tagline}>Your trusted Parenting Companion</Text>
              </View>
              {/* You can put a small icon here if you want */}
            </View>
          </LinearGradient>
        </View>

        {/* Sheet with fixed content (no scrolling) */}
        <View
          style={[
            styles.sheet,
            {
        ...
        >
          {/* Search pill row (opens map/add) */}
          <View style={styles.searchRow}>
            <TouchableOpacity style={styles.heroSearch} activeOpacity={0.9} onPress={() => setShowMapView(true)}>
              <MaterialIcons name="location-on" size={18} color="#9AA0A6" />
              <Text style={styles.heroSearchText}>Find nearby locations...</Text>
            </TouchableOpacity>

            <View style={styles.searchActions}>
              <TouchableOpacity onPress={() => navigation.navigate('LocationList', { locations, details })} style={styles.iconBtn} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                <MaterialIcons name="bookmark" size={22} color="#5973FF" />
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setShowMapView(true)} style={[styles.iconBtn, { marginLeft: 8 }]} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                <MaterialIcons name="add-location-alt" size={22} color="#5973FF" />
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.prefGrid}>
              <TouchableOpacity
                style={[styles.prefTile, contentPreferences.includes('language') && styles.prefTileActive]}
                onPress={() => navigation.navigate('ContentSelection')}
                activeOpacity={0.9}
              >
                <MaterialIcons name="chat" size={26} color={contentPreferences.includes('language') ? '#4A90E2' : '#9AA0A6'} />
                <Text style={[styles.prefTitle, contentPreferences.includes('language') && styles.prefTitleActive]}>Language</Text>
                <Text style={styles.prefSub}>Development</Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={[styles.prefTile, contentPreferences.includes('science') && styles.prefTileActive]}
                onPress={() => navigation.navigate('ContentSelection')}
                activeOpacity={0.9}
              >
                <MaterialIcons name="science" size={26} color={contentPreferences.includes('science') ? '#4A90E2' : '#9AA0A6'} />
                <Text style={[styles.prefTitle, contentPreferences.includes('science') && styles.prefTitleActive]}>Science</Text>
              ...
              <TouchableOpacity
                style={[
                  styles.prefTile,
                  !contentPreferences.includes('literacy') && styles.prefTileDisabled,
                  contentPreferences.includes('literacy') && styles.prefTileActive,
                ]}
                onPress={() => navigation.navigate('ContentSelection')}
                activeOpacity={0.9}
              >
                <MaterialIcons name="menu-book" size={26} color={contentPreferences.includes('literacy') ? '#4A90E2' : '#D1D5DB'} />
                <Text style={[styles.prefTitle, contentPreferences.includes('literacy') ? styles.prefTitleActive : styles.prefTitleMuted]}>Literacy</Text>
                <Text style={[styles.prefSub, styles.prefSubMuted]}>Soon</Text>
              </TouchableOpacity>

              <TouchableOpacity
                style={[
                  styles.prefTile,
                  !contentPreferences.includes('social') && styles.prefTileDisabled,
                  contentPreferences.includes('social') && styles.prefTileActive,
                ]}
                onPress={() => navigation.navigate('ContentSelection')}
                activeOpacity={0.9}
              >
                <MaterialIcons name="people" size={26} color={contentPreferences.includes('social') ? '#4A90E2' : '#D1D5DB'} />
                <Text style={[styles.prefTitle, contentPreferences.includes('social') ? styles.prefTitleActive : styles.prefTitleMuted]}>Social</Text>
                <Text style={[styles.prefSub, styles.prefSubMuted]}>Soon</Text>
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.card}>
            ...
            <TouchableOpacity activeOpacity={0.9} disabled={isAssistantLoading} onPress={getPersonalizedTips} style={{ borderRadius: 22, overflow: 'hidden' }}>
              <LinearGradient colors={['#3B82F6', '#7C4DFF']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.ctaGradient}>
                {isAssistantLoading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.ctaText}>Get Parenting Advice</Text>}
              </LinearGradient>
            </TouchableOpacity>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#E8E8E8' },
  ...
  appName: { paddingTop: 60, fontSize: 30, fontWeight: 'bold', color: '#fff', letterSpacing: 1 },
  tagline: { fontSize: 14, color: 'rgba(255,255,255,0.9)', marginTop: 4 },

  // Fixed sheet (no scroll)
  sheet: {
    position: 'absolute',
    left: 16,
    right: 16,
  ...
  searchActions: { flexDirection: 'row', marginLeft: 10 },
  iconBtn: {
    height: 44,
    width: 44,
    borderRadius: 22,
  ...
  micPill: { width: 32, height: 32, borderRadius: 16, backgroundColor: '#EEF2FF', alignItems: 'center', justifyContent: 'center' },

  // CTA
  ctaGradient: { height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  ctaText: { color: '#fff', fontWeight: '700', fontSize: 15 },

  // Small avatar dot
  avatarDot: { width: 28, height: 28, borderRadius: 14, backgroundColor: '#F1F5FF', borderWidth: 1, borderColor: '#E5E7EB' },
  ...
  pillItem: { flex: 1, height: 38, borderRadius: 22, alignItems: 'center', justifyContent: 'center' },
  ...
  loadingText: { marginTop: 20, fontSize: 16, color: '#333', textAlign: 'center', fontWeight: '500' },
  ...
  playButtonText: { color: 'white', fontSize: 12, fontWeight: '600', marginLeft: 4 },

  // Map modal
  mapModalContainer: { flex: 1, backgroundColor: '#fff' },
  mapHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: '#E8E8E8' },
  mapHeaderTitle: { fontSize: 18, fontWeight: '600', color: '#333' },
  fullMap: { flex: 1, zIndex: 0 },
  searchBarWrapper: { paddingHorizontal: 20, paddingTop: 8, paddingBottom: 4, backgroundColor: '#fff', zIndex: 10000, elevation: 10000 },
  searchInput: {
    height: 50,
    borderRadius: 12,
  ...
  dropdown: { height: 50, borderColor: '#E8E8E8', borderWidth: 1, borderRadius: 12, paddingHorizontal: 12, marginBottom: 16 },
  dropdownPlaceholder: { fontSize: 16, color: '#666' },
  dropdownSelected: { fontSize: 16, color: '#333', fontWeight: '500' },
  input: { height: 50, borderColor: '#E8E8E8', borderWidth: 1, borderRadius: 12, paddingHorizontal: 16, fontSize: 16, color: '#333', backgroundColor: '#FFFFFF', marginBottom: 16 },
