...
//         </View>
//         <ScrollView style={styles.scrollView} contentContainerStyle={[styles.scrollContent, { paddingBottom: insets.bottom + 115 }]}>
...
const SettingsScreen: React.FC<SettingsScreenProps> = ({ navigation }) => {
  ...
  const insets = useSafeAreaInsets();
  // Calculate header height to cover status bar + header content
  ...
  return (
    <View style={styles.safeArea}>
      ...
      {/* Blue header covering full top area including status bar */}
      <View style={styles.headerShadow}>
        <LinearGradient 
          colors={['#4A90E2', '#7B68EE']} 
          start={{ x: 0, y: 0 }} 
          end={{ x: 1, y: 1 }} 
          style={[styles.gradientBackground, { height: headerHeight, paddingTop: insets.top }]}
        >
          <View style={[styles.headerContainer, { paddingTop: 16 }]}>
            <TouchableOpacity
              style={styles.backButton}
              onPress={() => navigation.goBack()}
            >
      ...
      <View style={[styles.contentArea, { marginTop: headerHeight }]}>
        <ScrollView 
